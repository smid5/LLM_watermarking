...
def simple_encoder(text, model, tokenizer):
    """
    Encoder function: Converts input text into embeddings using a weighted average of the model's
    last hidden state, prioritizing more recent tokens.
    """
    inputs = tokenizer(text, return_tensors="pt").to(model.device)
    with torch.no_grad():
        outputs = model(**inputs, output_hidden_states=True)
        embeddings = outputs.hidden_states[-1].squeeze(0)  # Remove batch dimension if present
        
        # Apply a weighted average where later tokens get higher weights
        weights = torch.linspace(0.1, 1.0, embeddings.size(0)).to(embeddings.device)
        weighted_embeddings = embeddings * weights.unsqueeze(1)  # Apply weights to embeddings
        embeddings = weighted_embeddings.sum(0) / weights.sum()  # Compute the weighted average
    return embeddings
