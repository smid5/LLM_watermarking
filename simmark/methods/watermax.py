...
class WaterMaxProcessor(torch.nn.Module):
    ...
    def forward(self, input_ids, logits):
        ...
        for batch in range(batch_size):
            ...
            best_signature = None  # ensure defined

            for c in candidate_ids.tolist():
                # 2. compute seed from causal prefix + candidate
                extended_ids = torch.cat([input_ids[batch], torch.tensor([c], device=device)])
                # NOTE: i am passing hash_idx=0 here as watermax uses a signle key, but multiple key
                # versions are possible i guess 
                seed_val = self.seed_function(extended_ids, 
                                              self.prior_tokens, 
                                              self.tokenizer, 
...
def watermax_detect(text, config):
    ...
    for i in range(len(ids)):
        ...
        seed_val = seed_function(prefix_ids, 
                                 prior_tokens, 
                                 tokenizer, 
                                 transformer_model, 
                                 hash_idx=0, 
                                 ...
                                 k=k, 
                                 b=b)
        if seed_val in seen_signatures:
            continue
        seen_signatures.add(seed_val)
        rng = np.random.default_rng(seed_val)
        u = rng.standard_normal()
        S += float(u)
        M += 1
